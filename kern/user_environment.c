...
{
    uint32* user_page_WS = kmalloc(numOfElements*sizeof(struct WorkingSetElement));
    return (void*)user_page_WS;
}


void * create_user_directory()
{
    uint32* user_directory = kmalloc(PAGE_SIZE);
    return (void*)user_directory;
}
...
void env_free(struct Env *e)
{
    ...
    for(int i = 0; i < USER_TOP; i+=PAGE_SIZE){
        ...
        if(ptr_page_table_temp != 0){
            struct Frame_Info *ptrFrameInfo = to_frame_info(kheap_physical_address((uint32)ptr_page_table_temp));
            ptrFrameInfo->environment = NULL;
            ptrFrameInfo->references = 0;
