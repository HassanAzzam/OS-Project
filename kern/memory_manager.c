...
{
    uint32* page_table = kmalloc(PAGE_SIZE);
    uint32 ret=kheap_physical_address((uint32)page_table);
    uint32 page_table_phy = CONSTRUCT_ENTRY(ret,PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
    for(int i = 0; i < 1024; i++){
        page_table[i] = 0;
    }
    tlbflush();
    ptr_page_directory[PDX(virtual_address)] = page_table_phy;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
    for(uint32 i = virtual_address; i < virtual_address + size; i += PAGE_SIZE){
        if(pf_add_empty_env_page(e, i, 0) == E_NO_PAGE_FILE_SPACE)
        {
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
    ...
    uint32 VirtAddr=ROUNDDOWN(virtual_address,PAGE_SIZE);
    for(uint32 i = VirtAddr; i < virtual_address + size; i+=(PAGE_SIZE)){//loop overall pageTables
        uint32* ptr = NULL;
        get_page_table(e->env_page_directory,(void*)i, &ptr);
        bool in=0;// to check page table is empty (i.e. not used)
        if(ptr!=0){
            int j;
            for(j=0;j<1024;j++){
                if((ptr[j]&PERM_PRESENT)==PERM_PRESENT){
                    in=1;
                    break;
                }
            }
            if(in==0){//3.Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
                struct Frame_Info *FrameInfo = to_frame_info(kheap_physical_address((uint32)ptr));//ptr is pointer to  the pageTable
                FrameInfo->environment=NULL;
